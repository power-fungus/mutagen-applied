...
#[cfg_attr(test, ::mutagen::mutate)] impl Resource {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Resource<T>
where
    T: NewService<
        Config = (),
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> HttpServiceFactory for Resource<T>
where
    T: NewService<
            Config = (),
            Request = ServiceRequest,
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> IntoNewService<T> for Resource<T>
where
    T: NewService<
        Config = (),
        Request = ServiceRequest,
...
#[cfg_attr(test, ::mutagen::mutate)] impl NewService for ResourceFactory {
    type Config = ();
...
#[cfg_attr(test, ::mutagen::mutate)] impl Future for CreateResourceService {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Service for ResourceService {
    type Request = ServiceRequest;
    type Response = ServiceResponse;
    type Error = Error;
    type Future = Either<
...
#[cfg_attr(test, ::mutagen::mutate)] impl ResourceEndpoint {
    fn new(factory: Rc<RefCell<Option<ResourceFactory>>>) -> Self {
        ResourceEndpoint { factory }
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl NewService for ResourceEndpoint {
    type Config = ();
    type Request = ServiceRequest;
    type Response = ServiceResponse;
    type Error = Error;
