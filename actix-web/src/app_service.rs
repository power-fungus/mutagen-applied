...
#[cfg_attr(test, ::mutagen::mutate)] impl<T, B> NewService for AppInit<T, B>
where
    T: NewService<
        Config = (),
        Request = ServiceRequest,
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T, B> Future for AppInitResult<T, B>
where
    T: NewService<
        Config = (),
        Request = ServiceRequest,
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T, B> Service for AppInitService<T, B>
where
    T: Service<Request = ServiceRequest, Response = ServiceResponse<B>, Error = Error>,
{
    type Request = Request;
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T, B> Drop for AppInitService<T, B>
where
    T: Service<Request = ServiceRequest, Response = ServiceResponse<B>, Error = Error>,
{
    fn drop(&mut self) {
...
#[cfg_attr(test, ::mutagen::mutate)] impl NewService for AppRoutingFactory {
    type Config = ();
...
#[cfg_attr(test, ::mutagen::mutate)] impl Future for AppRoutingFactoryResponse {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Service for AppRouting {
    type Request = ServiceRequest;
    type Response = ServiceResponse;
    type Error = Error;
...
#[cfg_attr(test, ::mutagen::mutate)] impl AppEntry {
    pub fn new(factory: Rc<RefCell<Option<AppRoutingFactory>>>) -> Self {
        AppEntry { factory }
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl NewService for AppEntry {
    type Config = ();
    type Request = ServiceRequest;
    type Response = ServiceResponse;
    type Error = Error;
