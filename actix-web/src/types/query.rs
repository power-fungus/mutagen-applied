...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Query<T> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> ops::Deref for Query<T> {
    type Target = T;
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> ops::DerefMut for Query<T> {
    fn deref_mut(&mut self) -> &mut T {
        &mut self.0
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T: fmt::Debug> fmt::Debug for Query<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.0.fmt(f)
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T: fmt::Display> fmt::Display for Query<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.0.fmt(f)
    }
}
...
/// }
/// ```
#[cfg_attr(test, ::mutagen::mutate)] impl<T> FromRequest for Query<T>
where
    T: de::DeserializeOwned,
{
    type Error = Error;
...
#[cfg_attr(test, ::mutagen::mutate)] impl QueryConfig {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Default for QueryConfig {
    fn default() -> Self {
