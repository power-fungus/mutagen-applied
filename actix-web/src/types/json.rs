...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Json<T> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> ops::Deref for Json<T> {
    type Target = T;
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> ops::DerefMut for Json<T> {
    fn deref_mut(&mut self) -> &mut T {
        &mut self.0
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T> fmt::Debug for Json<T>
where
    T: fmt::Debug,
{
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> fmt::Display for Json<T>
where
    T: fmt::Display,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T: Serialize> Responder for Json<T> {
    type Error = Error;
...
/// }
/// ```
#[cfg_attr(test, ::mutagen::mutate)] impl<T> FromRequest for Json<T>
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl JsonConfig {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Default for JsonConfig {
    fn default() -> Self {
        JsonConfig {
            limit: 32768,
            ehandler: None,
...
#[cfg_attr(test, ::mutagen::mutate)] impl<U> JsonBody<U>
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl<U> Future for JsonBody<U>
where
    U: DeserializeOwned + 'static,
{
    type Item = U;
