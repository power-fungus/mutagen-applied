...
#[cfg_attr(test, ::mutagen::mutate)] impl Scope {
    /// Create a new scope
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Scope<T>
where
    T: NewService<
        Config = (),
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> HttpServiceFactory for Scope<T>
where
    T: NewService<
            Config = (),
            Request = ServiceRequest,
...
#[cfg_attr(test, ::mutagen::mutate)] impl NewService for ScopeFactory {
    type Config = ();
...
#[cfg_attr(test, ::mutagen::mutate)] impl Future for ScopeFactoryResponse {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Service for ScopeService {
    type Request = ServiceRequest;
    type Response = ServiceResponse;
    type Error = Error;
...
#[cfg_attr(test, ::mutagen::mutate)] impl ScopeEndpoint {
    fn new(factory: Rc<RefCell<Option<ScopeFactory>>>) -> Self {
        ScopeEndpoint { factory }
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl NewService for ScopeEndpoint {
    type Config = ();
    type Request = ServiceRequest;
    type Response = ServiceResponse;
    type Error = Error;
