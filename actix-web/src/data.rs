...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Data<T> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Deref for Data<T> {
    type Target = T;
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Clone for Data<T> {
    fn clone(&self) -> Data<T> {
        Data(self.0.clone())
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T: 'static> FromRequest for Data<T> {
    type Config = ();
    type Error = Error;
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T: 'static> DataFactory for Data<T> {
    fn create(&self, extensions: &mut Extensions) -> bool {
