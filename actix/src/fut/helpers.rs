...
#[cfg_attr(test, ::mutagen::mutate)] impl<S: Stream> FinishStream for S {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<S> Finish<S> {
    pub fn new(s: S) -> Finish<S> {
        Finish(s)
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<S> Future for Finish<S>
where
    S: Stream,
{
    type Item = ();
