...
#[cfg_attr(test, ::mutagen::mutate)] impl<F: ActorFuture> IntoActorFuture for F {
    type Future = F;
    type Item = F::Item;
    type Error = F::Error;
    type Actor = F::Actor;
...
#[cfg_attr(test, ::mutagen::mutate)] impl<F: ActorFuture + ?Sized> ActorFuture for Box<F> {
    type Item = F::Item;
    type Error = F::Error;
    type Actor = F::Actor;
...
#[cfg_attr(test, ::mutagen::mutate)] impl<F: Future, A: Actor> WrapFuture<A> for F {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<F, A> ActorFuture for FutureWrap<F, A>
where
    F: Future,
    A: Actor,
{
...
#[cfg_attr(test, ::mutagen::mutate)] impl<S: Stream, A: Actor> WrapStream<A> for S {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<S, A> ActorStream for StreamWrap<S, A>
where
    S: Stream,
    A: Actor,
{
