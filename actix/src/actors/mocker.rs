...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Mocker<T> {
    pub fn mock(
        mock: Box<dyn FnMut(Box<dyn Any>, &mut Context<Mocker<T>>) -> Box<dyn Any>>,
    ) -> Mocker<T> {
        Mocker::<T> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T: SystemService> SystemService for Mocker<T> {}
#[cfg_attr(test, ::mutagen::mutate)] impl<T: ArbiterService> ArbiterService for Mocker<T> {}
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Supervised for Mocker<T> {}

#[cfg_attr(test, ::mutagen::mutate)] impl<T> Default for Mocker<T> {
    fn default() -> Self {
        panic!("Mocker actor used before set")
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T: Sized + 'static> Actor for Mocker<T> {
    type Context = Context<Self>;
}

#[cfg_attr(test, ::mutagen::mutate)] impl<M: 'static, T: Sized + 'static> Handler<M> for Mocker<T>
where
    M: Message,
