...
pub fn number(s: &str, min: usize, max: usize) -> ParseResult<(&str, i64)> {
    ...
    let v: i64 = r#try!(s[..upto].parse().map_err(|_| OUT_OF_RANGE));
    Ok((&s[upto..], v))
}
...
pub fn nanosecond(s: &str) -> ParseResult<(&str, i64)> {
    ...
    let origlen = s.len();
    let (s, v) = r#try!(number(s, 1, 9));
    let consumed = origlen - s.len();

    // scale the number accordingly.
    static SCALE: [i64; 10] = [0, 100_000_000, 10_000_000, 1_000_000, 100_000, 10_000,
                               1_000, 100, 10, 1];
    let v = r#try!(v.checked_mul(SCALE[consumed]).ok_or(OUT_OF_RANGE));
...
pub fn nanosecond_fixed(s: &str, digits: usize) -> ParseResult<(&str, i64)> {
    // record the number of digits consumed for later scaling.
    let (s, v) = r#try!(number(s, digits, digits));

    // scale the number accordingly.
    static SCALE: [i64; 10] = [0, 100_000_000, 10_000_000, 1_000_000, 100_000, 10_000,
                               1_000, 100, 10, 1];
    let v = r#try!(v.checked_mul(SCALE[digits]).ok_or(OUT_OF_RANGE));
...
pub fn short_or_long_month0(s: &str) -> ParseResult<(&str, u8)> {
    ...
    let (mut s, month0) = r#try!(short_month0(s));
...
pub fn short_or_long_weekday(s: &str) -> ParseResult<(&str, Weekday)> {
    ...
    let (mut s, weekday) = r#try!(short_weekday(s));
    ...
    where F: FnMut(&str) -> ParseResult<&str>
{
    ...
    // hours (00--99)
    let hours = match r#try!(digits(s)) {
...
pub fn timezone_offset_2822(s: &str) -> ParseResult<(&str, Option<i32>)> {
    ...
    if upto > 0 {
        ...
        let (s_, offset) = r#try!(timezone_offset(s, |s| Ok(s)));
