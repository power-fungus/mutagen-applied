...
#[cfg_attr(test, ::mutagen::mutate)] impl<E> Fallback<E> {
    fn to_h2(&self) -> bool {
        match *self {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<E> Unpin for Fallback<E> {}
...
#[cfg_attr(test, ::mutagen::mutate)] impl Http {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<E> Http<E> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<I, B, S, E> Connection<I, S, E>
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl<I, B, S, E> Future for Connection<I, S, E>
where
    S: Service<Body, ResBody=B>,
    S::Error: Into<Box<dyn StdError + Send + Sync>>,
...
#[cfg_attr(test, ::mutagen::mutate)] impl<I, S> fmt::Debug for Connection<I, S>
where
    S: Service<Body>,
{
...
#[cfg_attr(test, ::mutagen::mutate)] impl<I, S, E> Serve<I, S, E> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<I, IO, IE, S, B, E> Serve<I, S, E>
where
    I: Accept<Conn=IO, Error=IE>,
    IO: AsyncRead + AsyncWrite + Unpin,
    IE: Into<Box<dyn StdError + Send + Sync>>,
...
// deprecated
#[cfg_attr(test, ::mutagen::mutate)] impl<I, IO, IE, S, B, E> Stream for Serve<I, S, E>
where
    I: Accept<Conn=IO, Error=IE>,
    IO: AsyncRead + AsyncWrite + Unpin,
    IE: Into<Box<dyn StdError + Send + Sync>>,
...
#[cfg_attr(test, ::mutagen::mutate)] impl<I, F, S, FE, E, B> Future for Connecting<I, F, E>
where
    I: AsyncRead + AsyncWrite + Unpin,
...
#[cfg(feature = "tcp")]
#[cfg_attr(test, ::mutagen::mutate)] impl<S, E> SpawnAll<AddrIncoming, S, E> {
    pub(super) fn local_addr(&self) -> SocketAddr {
        self.serve.incoming.local_addr()
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<I, S, E> SpawnAll<I, S, E> {
    pub(super) fn incoming_ref(&self) -> &I {
        self.serve.incoming_ref()
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<I, IO, IE, S, B, E> SpawnAll<I, S, E>
where
    I: Accept<Conn=IO, Error=IE>,
    IE: Into<Box<dyn StdError + Send + Sync>>,
...
#[cfg_attr(test, ::mutagen::mutate)] impl<A, B> Future for Either<A, B>
where
    A: Future,
