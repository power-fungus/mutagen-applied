...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Payload for T
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> sealed::Sealed for T
where
    T: HttpBody + Send + 'static,
    T::Data: Send,
    T::Error: Into<Box<dyn StdError + Send + Sync>>,
...
/*
#[cfg_attr(test, ::mutagen::mutate)] impl<E: Payload> Payload for Box<E> {
    type Data = E::Data;
    type Error = E::Error;
