...
#[cfg_attr(test, ::mutagen::mutate)] impl<D, Bs, I, T> Dispatcher<D, Bs, I, T>
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl<D, Bs, I, T> Future for Dispatcher<D, Bs, I, T>
where
    D: Dispatch<PollItem=MessageHead<T::Outgoing>, PollBody=Bs, RecvItem=MessageHead<T::Incoming>> + Unpin,
    D::PollError: Into<Box<dyn StdError + Send + Sync>>,
    I: AsyncRead + AsyncWrite + Unpin,
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'a, T> OptGuard<'a, T> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'a, T> Drop for OptGuard<'a, T> {
    fn drop(&mut self) {
        if self.1 {
            self.0.set(None);
        }
...
#[cfg_attr(test, ::mutagen::mutate)] impl<S, B> Server<S, B>
where
    S: Service<B>,
{
...
#[cfg_attr(test, ::mutagen::mutate)] impl<S: Service<B>, B> Unpin for Server<S, B> {}

#[cfg_attr(test, ::mutagen::mutate)] impl<S, Bs> Dispatch for Server<S, Body>
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl<B> Client<B> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<B> Dispatch for Client<B>
where
    B: Payload,
{
