...
#[cfg_attr(test, ::mutagen::mutate)] impl<T: Connect + ?Sized> Connect for Box<T> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Destination {
...
#[cfg_attr(test, ::mutagen::mutate)] impl TryFrom<Uri> for Destination {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Connected {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Extra {
    pub(super) fn set(&self, res: &mut Response<crate::Body>) {
        self.0.set(res);
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl Clone for Extra {
    fn clone(&self) -> Extra {
        Extra(self.0.clone_box())
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl fmt::Debug for Extra {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> ExtraInner for ExtraEnvelope<T>
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T: Clone> Clone for ExtraChain<T> {
    fn clone(&self) -> Self {
        ExtraChain(self.0.clone_box(), self.1.clone())
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T> ExtraInner for ExtraChain<T>
where
    T: Clone + Send + Sync + 'static
{
    fn clone_box(&self) -> Box<dyn ExtraInner> {
