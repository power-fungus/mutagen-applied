...
#[cfg_attr(test, ::mutagen::mutate)] impl<F, R> Started for Lazy<F, R>
where
    F: FnOnce() -> R,
    R: Future + Unpin,
{
...
#[cfg_attr(test, ::mutagen::mutate)] impl<F, R> Future for Lazy<F, R>
where
    F: FnOnce() -> R,
    R: Future + Unpin,
{
...
#[cfg_attr(test, ::mutagen::mutate)] impl<F, R: Unpin> Unpin for Lazy<F, R> {}
