...
#[cfg_attr(test, ::mutagen::mutate)] impl<T, B1, B2> Service<B1> for T
where 
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T, B1, B2> sealed::Sealed<B1> for T 
where 
    T: tower_service::Service<Request<B1>, Response = Response<B2>>,
    B2: Payload,
{}
...
/// });
/// ```
#[cfg_attr(test, ::mutagen::mutate)] pub fn service_fn<F, R, S>(f: F) -> ServiceFn<F, R>
where
    F: FnMut(Request<R>) -> S,
    S: Future,
{
...
#[cfg_attr(test, ::mutagen::mutate)] impl<F, ReqBody, Ret, ResBody, E> tower_service::Service<crate::Request<ReqBody>> for ServiceFn<F, ReqBody>
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl<F, R> fmt::Debug for ServiceFn<F, R> {
