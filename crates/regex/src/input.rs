...
#[cfg_attr(test, ::mutagen::mutate)] impl InputAt {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'a, T: Input> Input for &'a T {
    fn at(&self, i: usize) -> InputAt {
        (**self).at(i)
    }
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'t> CharInput<'t> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'t> ops::Deref for CharInput<'t> {
    type Target = [u8];
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'t> Input for CharInput<'t> {
    fn at(&self, i: usize) -> InputAt {
        if i >= self.len() {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'t> ByteInput<'t> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'t> ops::Deref for ByteInput<'t> {
    type Target = [u8];
...
#[cfg_attr(test, ::mutagen::mutate)] impl<'t> Input for ByteInput<'t> {
    fn at(&self, i: usize) -> InputAt {
        if i >= self.len() {
            InputAt { pos: self.len(), c: None.into(), byte: None, len: 0 }
        } else {
...
#[cfg_attr(test, ::mutagen::mutate)] impl fmt::Debug for Char {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Char {
...
#[cfg_attr(test, ::mutagen::mutate)] impl From<char> for Char {
    fn from(c: char) -> Char {
        Char(c as u32)
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl From<Option<char>> for Char {
    fn from(c: Option<char>) -> Char {
        c.map_or(Char(u32::MAX), |c| c.into())
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl PartialEq<char> for Char {
    #[inline]
...
#[cfg_attr(test, ::mutagen::mutate)] impl PartialOrd<char> for Char {
    #[inline]
