...
#[cfg_attr(test, ::mutagen::mutate)] impl<'a> From<&'a str> for Ref<'a> {
    fn from(x: &'a str) -> Ref<'a> {
        Ref::Named(x)
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl From<usize> for Ref<'static> {
...
#[cfg_attr(test, ::mutagen::mutate)] fn find_cap_ref<T: ?Sized + AsRef<[u8]>>(
    replacement: &T,
) -> Option<CaptureRef> {
    let mut i = 0;
...
#[cfg_attr(test, ::mutagen::mutate)] fn is_valid_cap_letter(b: &u8) -> bool {
    match *b {
