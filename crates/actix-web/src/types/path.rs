...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> Path<T> {
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> AsRef<T> for Path<T> {
    fn as_ref(&self) -> &T {
        &self.inner
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T> ops::Deref for Path<T> {
    type Target = T;
...
#[cfg_attr(test, ::mutagen::mutate)] impl<T> ops::DerefMut for Path<T> {
    fn deref_mut(&mut self) -> &mut T {
        &mut self.inner
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T> From<T> for Path<T> {
    fn from(inner: T) -> Path<T> {
        Path { inner }
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T: fmt::Debug> fmt::Debug for Path<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.inner.fmt(f)
    }
}

#[cfg_attr(test, ::mutagen::mutate)] impl<T: fmt::Display> fmt::Display for Path<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        self.inner.fmt(f)
    }
}
...
/// }
/// ```
#[cfg_attr(test, ::mutagen::mutate)] impl<T> FromRequest for Path<T>
where
...
#[cfg_attr(test, ::mutagen::mutate)] impl PathConfig {
...
#[cfg_attr(test, ::mutagen::mutate)] impl Default for PathConfig {
    fn default() -> Self {
